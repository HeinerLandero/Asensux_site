...
import { Cpu, Palette, CloudCog } from "lucide-react";
import { ease, easeAlt } from "../../lib/animations";
...
export default function AboutSection() {
  return (
    <section
      id="about"
      className="   from-navyDark to-navy text-antiFlashWhite py-16 sm:py-24 md:py-32 px-4 sm:px-6 justify-center items-center"
    >
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <h2 className="text-2xl sm:text-3xl md:text-5xl font-bold leading-tight">
            Ingeniería, diseño e infraestructura{" "}
            <span className="text-electricBlue">en un solo equipo</span>.
          </h2>
          <p className="mt-4 sm:mt-6 text-base sm:text-lg text-stone/70 max-w-2xl mx-auto leading-relaxed">
          No tercerizamos ni fragmentamos. Todo lo que construimos pasa por el mismo equipo: desde la arquitectura hasta el píxel final.
          </p>
        </motion.div>
        ...
        <div className="space-y-3 sm:space-y-4">
          {features.map((f, i) => (
            <motion.div
              key={f.title}
              className="group flex items-start gap-4 sm:gap-6 p-4 sm:p-6 sm:p-8 rounded-xl sm:rounded-2xl border border-white/5 hover:border-electricBlue/20  /30 hover: /50 transition-all duration-500"
              initial={{ opacity: 0, x: i % 2 === 0 ? -30 : 30 }}
              whileInView={{ opacity: 1, x: 0 }}
              transition={{ delay: i * 0.1, duration: 0.6, ease: easeAlt }}
              viewport={{ once: true }}
            >
